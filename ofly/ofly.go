...
import (
	...
	"github.com/opsbyte/ofly"
	"github.com/opsbyte/ofly/internal"
...
func main() {
	if os.Getenv("OFLY_RUN_SERVER") == "true" {
		oflyServer()
		return
	}
	if len(os.Args) > 1 && (os.Args[1] == "oflys" || os.Args[1] == "/bin/oflys") {
		// Reproduce the older docker environment
		os.Args = append(os.Args[:1], os.Args[2:]...)
		if os.Getenv("OFLY_KEY") == "" {
			os.Setenv("OFLY_KEY", "oflys")
		}
		oflyServer()
		return
	} else if len(os.Args) > 1 && os.Args[1] == "ofly" {
		os.Args = append(os.Args[:1], os.Args[2:]...)
	}
	flag.Parse()
	...
	if port := *portFlag; port > 0 {
		...
		if os.Getenv("OFLY_KEY") == "" {
			os.Setenv("OFLY_KEY", fmt.Sprintf("p%d", port))
		}
	} else {
...
func authValidator() func(username, password string) bool {
	...
	log.Println("ofly: using plain text password")
	return func(username, password string) bool {
